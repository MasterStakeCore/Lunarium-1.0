...
// Copyright (c) 2017-2018 The Lunarium developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zxlncontroldialog.h"
#include "ui_zxlncontroldialog.h"
...
std::set<std::string> ZXlnControlDialog::setSelectedMints;
std::set<CMintMeta> ZXlnControlDialog::setMints;

ZXlnControlDialog::ZXlnControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZXlnControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZXlnControlDialog::~ZXlnControlDialog()
{
    delete ui;
}

void ZXlnControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZXlnControlDialog::updateList()
{
...
void ZXlnControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZXlnControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZXln_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZXlnControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZXlnControlDialog::GetSelectedMints()
{
...
void ZXlnControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
