...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered lunarium address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 XLN.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal lunarium:URI (lunarium:XyZ...?message=example)
            {
...
static void NotifyzXLNReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzXLNReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzXLNReset.connect(boost::bind(NotifyzXLNReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzXLNReset.disconnect(boost::bind(NotifyzXLNReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zxlnTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
