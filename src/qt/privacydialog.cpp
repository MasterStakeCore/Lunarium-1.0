                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zXLN ought to be enough for anybody." - Bill Gates, 2017
    ui->zXLNpayAmount->setValidator( new QDoubleValidator(0.0, 14190000.0, 20, this) );
    ...
    ui->labelzXLNSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zXLNpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzXLN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXLN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zXLN, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zXLN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zXLN in ") +
...
void PrivacyDialog::on_pushButtonSpendzXLN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXLN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zXLN, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zXLN
        sendzXLN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zXLN
    sendzXLN();
}

void PrivacyDialog::on_pushButtonZXlnControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZXlnControlDialog* zXlnControl = new ZXlnControlDialog(this);
    zXlnControl->setModel(walletModel);
    zXlnControl->exec();
}

void PrivacyDialog::setZXlnControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzXlnSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzXLN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Lunarium Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zXLNpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zXLN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " XLN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zXLN</b>";
    ...
    // use mints from zXLN selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZXlnControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZXlnControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zXLN require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zXLN"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZXLN_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zXLN require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zXLN"));
            ui->TEMintStatus->repaint();
            return;
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zXLN transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zXLN was spent successfully update the addressbook with the label
    ...
    // Clear zxln selector in case it was used
    ZXlnControlDialog::setSelectedMints.clear();
    ui->labelzXlnSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zXLN Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zxlnTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zXLN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zXLN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zXLN "));
    ui->labelzXLNAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zXLN </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zXLN </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzXLNSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzXLN->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zXLN
        ui->pushButtonMintzXLN->setEnabled(false);
        ui->pushButtonMintzXLN->setToolTip(tr("zXLN is currently disabled due to maintenance."));

        // Spend zXLN
        ui->pushButtonSpendzXLN->setEnabled(false);
        ui->pushButtonSpendzXLN->setToolTip(tr("zXLN is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zXLN
        ui->pushButtonMintzXLN->setEnabled(true);
        ui->pushButtonMintzXLN->setToolTip(tr("PrivacyDialog", "Enter an amount of XLN to convert to zXLN", 0));

        // Spend zXLN
        ui->pushButtonSpendzXLN->setEnabled(true);
        ui->pushButtonSpendzXLN->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
