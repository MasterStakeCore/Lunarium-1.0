...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::XLN);
    ...
    if (!settings.contains("nAnonymizeLunariumAmount"))
        settings.setValue("nAnonymizeLunariumAmount", 1000);

    nAnonymizeLunariumAmount = settings.value("nAnonymizeLunariumAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeLunariumAmount"))
        SoftSetArg("-anonymizelunariumamount", settings.value("nAnonymizeLunariumAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in lunarium.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLunariumAmount:
            return QVariant(nAnonymizeLunariumAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLunariumAmount:
            nAnonymizeLunariumAmount = value.toInt();
            settings.setValue("nAnonymizeLunariumAmount", nAnonymizeLunariumAmount);
            emit anonymizeLunariumAmountChanged(nAnonymizeLunariumAmount);
            break;
        case CoinControlFeatures:
