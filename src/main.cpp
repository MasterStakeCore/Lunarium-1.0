...
#if defined(NDEBUG)
#error "Lunarium cannot be compiled without assertions."
#endif
...
{
    //Check to see if the zXLNis properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zXLNspend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zXLNwithout the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zXLNspend with serial %s is already in block %d\n", __func__,
    ...
    if (!spend.HasValidSerial(Params().Zerocoin_Params(fUseV1Params)))
        return error("%s : zXLNspend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zXLNspend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zxln");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zXLNmints are not already known
...
CAmount GetCurrentCollateral()
{
    ...
    if (blockHeight >  1024400) return 200000 ;  
    if (blockHeight >   761600) return 200000 ;  
    if (blockHeight >   498800) return 150000 ;  
    if (blockHeight >   236000) return 120000 ;   
    if (blockHeight >      200) return 100000 ;  
...
int64_t GetBlockValue(int nHeight)
{
    int64_t nSubsidy;
    if (nHeight == 0) {
    nSubsidy = COIN * 30000000;
    } else if (nHeight < Params().LAST_POW_BLOCK() && nHeight >= 0) {
        nSubsidy = COIN * 4;
    } else if ( nHeight >= Params().LAST_POW_BLOCK() && nHeight <= 21600) {
        nSubsidy = COIN * 4;
    } else if (nHeight > Params().SubsidyHalvingBlock() && nHeight >= 21601 && nHeight <= 236000) { 
        nSubsidy = COIN * 25;
    } else if (nHeight > Params().SubsidyHalvingBlock() && nHeight >= 236001 && nHeight <= 498800) {
        nSubsidy = COIN * 50;
    } else if (nHeight > Params().SubsidyHalvingBlock() && nHeight >= 498801 && nHeight <= 761600) {
        nSubsidy = COIN * 60;
    } else if (nHeight > Params().SubsidyHalvingBlock() && nHeight >= 761601 && nHeight <= 1024400) {
        nSubsidy = COIN * 70;
    } else if (nHeight > Params().SubsidyHalvingBlock() && nHeight >= 1024400 && nHeight <= 5000000) {
        nSubsidy = COIN * 80;
    } else {
        nSubsidy = COIN * 4;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, bool isZXLNStake)
{
    int64_t nMNSubsidy;
    if(nHeight < Params().LAST_POW_BLOCK()){
        nMNSubsidy = COIN * 0;
    } else if (nHeight >= Params().LAST_POW_BLOCK() && nHeight <= 236000){
        nMNSubsidy = blockValue *  0.80;
    } else if (nHeight > Params().SubsidyHalvingBlock() && nHeight >= 236001 && nHeight <= 5000000) {    
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Lunarium
...
{
    RenameThread("lunarium-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZXLNMinted()
{
...
bool RecalculateXLNSupply(int nHeightStart)
{
...
bool UpdateZXLNSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    //Track zXLNmoney supply in the block index
    if (!UpdateZXLNSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zXLNsupply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zXlnSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert XLNto zXLN
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zXLNspends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zXLNserial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zXLNstake block is less than allowed start height", __func__);

    if (CZXlnStake* zXLN= dynamic_cast<CZXlnStake*>(stake)) {
        CBlockIndex* pindexFrom = zXLN->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zXLNstake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zXLNstake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zXLN->GetValue()));
        if (nChecksum200 != zXLN->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zXLN->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZXLN() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zXLNfails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zXLNmints and %d zXLNspends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Lunarium: We use certain sporks during IBD, so check to see if they are
